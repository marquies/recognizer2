...
//            final Yolov3 yolo_model) {
//        final TFObjectDetector d = new TFObjectDetector();
//        d.inference = new TensorFlowInferenceInterface(yolo_model.getGraph());
//    )
    ...
    public List<Recognition> recognizeImage(final BufferedImage image) {
        ...
        for (int i = 0; i < outputScores.length; ++i) {
            ...
            if (outputScores[i] > 0.5) {
                System.out.println("XoXo: " + i + " ->" +outputClasses[i]);
                Recognition r = new Recognition("" + i, labels.get((int) outputClasses[i]), outputScores[i], detection);
